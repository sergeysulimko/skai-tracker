...
2. Прочитай последние 4 файла `digests/*-ai-weekly.md` (если есть) через Glob + Read. Запомни темы для дедупликации. ДОПОЛНИТЕЛЬНО: проверь Watch list из прошлонедельного digest'а — если что-то из ожидаемых событий случилось на этой неделе, включи в соответствующую категорию как «Обновление: [событие из прошлого Watch list] произошло — <детали>».
...
2. **Минимум 2 НЕЗАВИСИМЫХ источника для ВСЕХ новостей** (не только 🔴).
   - «Независимые» = разные организации. Две статьи разных изданий от одного автора ≠ 2 источника. Статьи, ссылающиеся друг на друга как primary source = 1 источник.
   - Исключение: official announcement от лабы/компании (anthropic.com/news, openai.com/blog, deepmind.google, ai.meta.com, x.ai/news, blogs.nvidia.com, huggingface.co/blog и т.п.) = 1 первичный источник. Для 🟡/⚪ этого достаточно. Для 🔴 всё равно нужно дополнительное press-подтверждение (≥1 независимый обзор в техмедиа).
   - Если нашёл только 1 источник и это НЕ first-party official — **ДРОП item**. Не включай. Кросс-проверка важнее объёма.
   - Перед включением любого кандидата сделай минимум 3 дополнительных WebSearch с разных углов (разные ключевые слова, разные временные рамки, поиск критических/skeptical мнений).
3. Это не ретроспектива, не обзор, не «итоги».
4. Проходит анти-hype фильтр: игнорировать «революционный», «первый в мире», «уникальный» без конкретных метрик или независимой верификации.
5. Если новость содержит конкретные цифры/метрики/бенчмарки — эти цифры должны совпадать в 2+ источниках. Если цифры расходятся или встречаются только в 1 источнике — не цитируй конкретные числа, используй обобщённую формулировку («значительное снижение», «улучшение на бенчмарках»).
...
- ПОКАЗЫВАЙ ВСЕ найденные источники (2-4 штуки) через · (средняя точка U+00B7) — для ЛЮБОЙ новости, не только 🔴. Минимум 2 независимых — требование. Формат: `<a href="URL1">источник →</a> · <a href="URL2">ещё →</a> · <a href="URL3">также →</a>`. Если первый источник — first-party official (лаба о себе), пометь его явно: `<a href="URL">anthropic.com (официально) →</a> · <a href="URL2">TechCrunch →</a>`.
...
Показывай ВСЕ найденные источники (2-4) через `·` для ЛЮБОЙ новости. Минимум 2 независимых — требование, не опция.
